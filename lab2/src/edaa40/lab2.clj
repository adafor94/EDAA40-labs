...
(defn surf 
  ...
  (if                               ;if  
    (or 
      (no-links? web current)       ; current page has no links
      (< (rand) (- 1 alpha)))       ; or rand < (1-alpha)
        (random-page web)           ; return a random page
        (random-link web current))  ; else return random link on current page
